...
from .utils import get_pose, set_pose, get_movable_joints, \
    ...
    step_simulation, refine_path, plan_direct_joint_motion, get_joint_positions, dump_world, wait_if_gui, flatten, get_all_links, get_link_names, Euler
...
GRASP_INFO = {
    'top': GraspInfo(lambda body: get_top_grasps(body, under=True, tool_pose=Pose(), max_width=INF,  grasp_length=-0.015),
                     approach_pose=Pose(0.1*Point(z=1.0))),
}
...
TOOL_FRAMES = {
    ...
    # 'panda': 'panda_link7', # iiwa_link_ee | iiwa_link_ee_kuka
}
...
def get_ik_fn(robot, fixed=[], teleport=False, num_attempts=10):
    ...
    def fn(body, pose, grasp):
        ...
        for i in range(num_attempts):
            ...
            # q_approach = next(ikfast_inverse_kinematics(robot, PANDA_INFO, grasp.link, approach_pose, max_attempts=10), None)
            q_approach = inverse_kinematics(robot, grasp.link, approach_pose)
            # print('joints: ', q_approach, ' Collision: ', any(pairwise_collision(robot, b) for b in obstacles))
            if (q_approach is None) or any(pairwise_collision(robot, b) for b in obstacles):
                # wait_if_gui('Collision Approach')
                continue
            conf = BodyConf(robot, q_approach)
            q_grasp = inverse_kinematics(robot, grasp.link, gripper_pose)
            # print('joints: ', q_grasp, ' Collision: ', any(pairwise_collision(robot, b) for b in obstacles))
